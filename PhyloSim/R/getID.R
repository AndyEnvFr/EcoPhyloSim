...
#' @return Input object with \code{idMat} matrices added to each generation
...
getID.PhyloSim <- function(simu) {
  ...
  for (i in 2:length(simu$Output)) {
    unchanged <- simu$Output[[i]]$traitMat == simu$Output[[i - 1]]$traitMat
