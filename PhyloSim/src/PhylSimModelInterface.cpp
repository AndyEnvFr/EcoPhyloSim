...
//' @export
// [[Rcpp::export]]
List callModel(int x, int y, int dispersal, IntegerVector runs,
               double specRate, bool dens, bool env, bool neutral,
               bool mort, int mortStrength, bool repro,
               int dispersalCutoff, int densityCutoff, int seed,
               double envStrength, double compStrength, int fission,
               double redQueen, double redQueenStrength, int protracted,
               NumericVector airmatR, NumericVector soilmatR,
               bool prunePhylogeny, double nicheWidth,
               double densityNicheWidth, double pDDNicheWidth,
               double pDDStrength, bool positiveDens) {
#ifdef DEBUG
  ...
  debugFile << "bool positiveDens = " << positiveDens << ";"
            << std::endl;
  debugFile << "bool env = " << env << ";" << std::endl;
  debugFile << "bool neutral = " << neutral << ";" << std::endl;
  debugFile << "bool mort = " << mort << ";" << std::endl;
  debugFile << "int mortStrength = " << mortStrength << ";"
            << std::endl;
  debugFile << "bool repro = " << repro << ";" << std::endl;
  debugFile << "int dispersalCutoff = " << dispersalCutoff << ";"
            << std::endl;
  debugFile << "int densityCutoff = " << densityCutoff << ";"
            << std::endl;
  debugFile << "int seed = " << seed << ";" << std::endl;
  debugFile << "double envStrength = " << envStrength << ";"
            << std::endl;
  debugFile << "double compStrength = " << compStrength << ";"
            << std::endl;
  debugFile << "int fission = " << fission << ";" << std::endl;
  debugFile << "double redQueen = " << redQueen << ";" << std::endl;
  debugFile << "double redQueenStrength = " << redQueenStrength << ";"
            << std::endl;
  debugFile << "int protracted = " << protracted << ";" << std::endl;
  debugFile << "NumericVector airmatR = " << airmatR << ";"
            << std::endl;
  debugFile << "airmatR = " << airmatR.begin() << ", " << airmatR.end()
            << ";" << std::endl;
  debugFile << "NumericVector airmatR = " << soilmatR << ";"
            << std::endl;
  debugFile << "soilmatR = " << soilmatR.begin() << ", "
            << soilmatR.end() << ";" << std::endl;
  debugFile << "double nicheWidth = " << nicheWidth << ";" << std::endl;
  debugFile << "double densityNicheWidth = " << densityNicheWidth << ";"
            << std::endl;
  debugFile << "double pDDNicheWidth = " << pDDNicheWidth << ";"
            << std::endl;
  debugFile.close();
#endif
  ...
  PhylSimModel phylSimModel(
      x, y, dispersal, runs[nRuns - 1], specRate, dens, env, neutral,
      mort, mortStrength, repro, dispersalCutoff, densityCutoff,
      tempSaveLoc, envStrength, compStrength, fission, redQueen,
      redQueenStrength, protracted, airmat, soilmat, nicheWidth,
      densityNicheWidth, pDDNicheWidth, pDDStrength, positiveDens);
  ...
  for (int step = 0; step < nRuns; step++) {
    ...
    if (dispersal == 1) {
      for (int i = 0; i < x; i++) {
        for (int j = 0; j < y; j++) {
          specOut[indCounter] =
              phylSimModel.m_Global->m_Individuals[i][j]
                  .m_Species->m_ID;
          traitOut[indCounter] =
              phylSimModel.m_Global->m_Individuals[i][j].m_Mean;
          envOut[indCounter] =
              phylSimModel.m_Global->m_Environment[i * y + j].first;
          neutralOut[indCounter] =
              phylSimModel.m_Global->m_Individuals[i][j]
                  .m_NeutralMarker;
          compOut[indCounter] =
              phylSimModel.m_Global->m_Individuals[i][j]
...
#endif
      if (prunePhylogeny) {
        phyloPass = phylSimModel.m_Global->m_Phylogeny.writePhylogenyR(
            1, phylSimModel.m_Global->m_Phylogeny.m_PrunedPhylo);
      } else {
        phyloPass = phylSimModel.m_Global->m_Phylogeny.writePhylogenyR(
            1, phylSimModel.m_Global->m_Phylogeny.m_FullPhylogeny);
      }
#ifdef DEBUG
      ...
      for (int i = 0; i < x; i++) {
        for (int j = 0; j < y; j++) {
          specOut[indCounter] =
              phylSimModel.m_Local->m_Individuals[i][j].m_Species->m_ID;
          traitOut[indCounter] =
              phylSimModel.m_Local->m_Individuals[i][j].m_Mean;
          envOut[indCounter] =
              phylSimModel.m_Local->m_Environment[i * y + j].first;
          neutralOut[indCounter] =
              phylSimModel.m_Local->m_Individuals[i][j].m_NeutralMarker;
          compOut[indCounter] =
              phylSimModel.m_Local->m_Individuals[i][j]
                  .m_CompetitionMarker;
          indCounter++;
        }
      }
      ...
      if (prunePhylogeny) {
        phyloPass = phylSimModel.m_Local->m_Phylogeny.writePhylogenyR(
            1, phylSimModel.m_Local->m_Phylogeny.m_PrunedPhylo);
      } else {
        phyloPass = phylSimModel.m_Local->m_Phylogeny.writePhylogenyR(
            1, phylSimModel.m_Local->m_Phylogeny.m_FullPhylogeny);
      }
    ...
    Rcpp::List listResults =
        Rcpp::List::create(Rcpp::Named("Species") = specOut,
